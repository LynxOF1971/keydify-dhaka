...
(() => {
  ...
  const labels = { visits: 'Website visits', whatsapp: 'WhatsApp enquiries', messenger: 'Messenger enquiries' };
  function paint(root, stats) {
    root.querySelectorAll('[data-counter]').forEach(node => {
      const value = stats[node.dataset.counter];
  ...
  async function track(kind) {
    ...
    try {
      ...
      if (response.ok) paint(document, await response.json());
  ...
  else { read().then(stats => paint(section, stats)).catch(() => { counterPanel.hidden = true; section.querySelector('.counter-note').textContent = 'Activity counters are not available yet.'; }); track('visits'); }
